...
@section('content')
    ...
    @if($cat[0]->image)
            style="background: url({{URL::to('/').'/subcategories/'}}{{$cat[0]->image}})" class="w-100" height: 350px;object-fit: cover

    @else
    @php

        $demoImage=DB::table('demo_images')->where('title', '=', 'articlesubcategory')->get();
                                ...
                                <div class="mb-sm-0 col col-lg fcontrol position-relative filter_selectWrap filter_selectWrap2">
                                    <div class="select-floating">
                                        ...
                                        <select class="filter_select form-control" name="title" id="searchFilterSelect" onchange="location =  this.options[this.selectedIndex].value;">
                                            ...
                                            @foreach ($articlesubcat as $index => $item)
                                                <option value="{!! URL::to('/article/category/'.$item->blogcategory->slug .'/'.$item->slug) !!}" {{ (request()->input('title') == $item->title) ? 'selected' : '' }}>{{ $item->title }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
    ...
    <section class="pb-4 pb-lg-5 mt-3 our-process">
        ...
        <ul class="breadcumb_list mb-4 border-bottom pb-2">
            ...
            <li>/</li>
            <li><a href="{{ route('article.category',$articlecat[0]->slug) }}">{{$articlecat[0]->title}}</a></li>
            <li>/</li>
