        ...
        <div class="col-md-8 mx-auto">
            <div class="tile">
                ...
                <form action="{{ route('admin.news.update') }}" method="POST" role="form" enctype="multipart/form-data">
                	<input type="hidden" name="id" value="{{ $targetNews->id }}">
                    @csrf
                    <div class="tile-body">
                        <div class="form-group">
                            <label class="control-label" for="name">Title <span class="m-l-5 text-danger"> *</span></label>
                            <input class="form-control @error('title') is-invalid @enderror" type="text" name="title" id="title" value="{{ old('title', $targetNews->title) }}"/>
                                        ...
                                        <div class="mb-sm-0 col col-lg fcontrol position-relative filter_selectWrap filter_selectWrap2">
                                            <div class="select-floating-admin">
                                                ...
                                                <select class="filter_select form-control" name="state">
                                                    ...
                                                    <option value="{{$item->name}}" {{ ($item->name == $targetNews->state) ? 'selected' : '' }}>{{ $item->name }}</option>
                                                    @endforeach
                                                </select>
                                                @error('state')
                                    ...
                                    <div class="mb-sm-0 col col-lg fcontrol position-relative filter_selectWrap filter_selectWrap2">
                                        <div class="select-floating-admin">
                                            ...
                                            <select class="filter_select form-control" name="postcode">
                                                ...
                                                <option value="{{$item->pin}}" {{ ($item->pin == $targetNews->postcode) ? 'selected' : '' }}>{{ $item->pin }}</option>
                                                @endforeach
                                            </select>
                                            @error('postcode')
                                                <p class="small text-danger">{{ $message }}</p>
                        ...
                        <div class="tile-body">
                            <div class="form-group">
                                <div class="select-floating-admin">
                                    ...
                                    <option value="" {{ ($targetNews->suburb) ? 'selected' : '' }}>{{$targetNews->suburb ?? ''}}</option>
                                    </select>
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" rows="4" name="description" id="description">{{ old('description', $targetNews->description) }}</textarea>
                        </div>
                        <div class="form-group">
                            <div class="row">
                                <div class="col-md-2">
                                    @if ($targetNews->image != null)
                                        <figure class="mt-2" style="width: 80px; height: auto;">
                                            <img src="{{ $targetNews->image }}" id="blogImage" class="img-fluid" alt="img">
                                        </figure>
                                    @endif
                                </div>
                                <div class="col-md-10">
