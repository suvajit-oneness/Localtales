    ...
    <div class="app-title">
        <div class="row w-100">
            ...
            <div class="col-md-6 text-right">
                <a href="{{ route('admin.job.category.create') }}" class="btn btn-primary"><i class="fa fa-plus"></i> Add New</a>
                {{-- <a href="#csvUploadModal" data-toggle="modal" class="btn btn-primary "><i class="fa fa-cloud-upload"></i> CSV Upload</a>
                <a href="{{ route('admin.job.category.data.csv.export') }}" class="btn btn-primary "><i class="fa fa-cloud-download"></i> CSV Export</a> --}}
            </div>
        </div>
    </div>
            ...
            <div class="row align-items-center justify-content-between">
                <div class="col">
                    <ul class="text-right mt-3">
                        <p class="font-weight : bold">Total Categories <span class="count">({{$categories->total()}})</span></p>
                    </ul>
                </div>
                <div class="col-auto">
                    <form action="{{ route('admin.job.category.index') }}">
                            ...
                            @foreach($categories as $key => $category)
                                ...
                                <td class="text-center">
                                    <div class="btn-group" role="group" aria-label="Second group">
                                        <a href="{{ route('admin.job.category.edit', $category['id']) }}" class="btn btn-sm btn-primary edit-btn"><i class="fa fa-edit"></i></a>
                                        <a href="{{ route('admin.job.category.details', $category['id']) }}" class="btn btn-sm btn-primary edit-btn"><i class="fa fa-eye"></i></a>
    ...
    <script type="text/javascript">
        $('input[id="toggle-block"]').change(function() {
          ...
          $.ajax({
                type:'POST',
                dataType:'JSON',
                url:"{{route('admin.job.category.updateStatus')}}",
