    ...
    <div class="app-title">
        <div class="row w-100">
            ...
            <div class="col-md-6 text-right">
                ...
                <a href="{{ route('admin.blog.data.csv.export',['term'=>$request->term]) }}" class="btn btn-primary "><i class="fa fa-cloud-download"></i> CSV Export</a>
            </div>
        </div>
    </div>
				     ...
				     <td>{{ $blog->subcategorylevel ? $blog->subcategorylevel->title : '' }}</td>
                                ...
                                {{-- <td class="text-center">
                                    <div class="toggle-button-cover margin-auto">
                                        <div class="button-cover">
                                            <div class="button-togglr b2" id="button-11">
                                                <input id="toggle-block" type="checkbox" name="blog_status" class="checkbox" data-blog_id="{{ $blog['id'] }}" {{ $blog['blog_status'] == 1 ? 'checked' : '' }}>
                                ...
                                <td class="text-center">
                                    <div class="btn-group" role="group" aria-label="Second group">
                                       ...
                                       <!-- <a href=" {{ route('admin.blogfaq.index', $blog->id) }}" type="button"  class="btn btn-sm btn-primary edit-btn"> Faq</a>-->
                                    </div>
                                </td>
                                </tr>
                            @endforeach
...
@push('scripts')
    ...
    <script type="text/javascript">
        $('input[id="toggle-block"]').change(function() {
            var blog_status_id = $(this).data('blog_id');
            var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
            var check_status = 0;
          if($(this).is(":checked")){
              check_status = 1;
          ...
          $.ajax({
                type:'POST',
                dataType:'JSON',
                url:"{{route('admin.blogStatus.updateStatus')}}",
