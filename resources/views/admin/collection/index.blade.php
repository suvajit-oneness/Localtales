    ...
    <div class="row">
        <div class="col-md-12">
            <div class="row mx-2">
                <div class="col">
                    <ul class="pl-0">
                        <p>Total Number of Collections <span class="count">({{$col->total()}})</span></p>
                        {{-- @php
                    ...
                    <table class="table table-hover custom-data-table-style table-striped">
                        <thead>
                            <tr>
                                ...
                                <th>Suburb</th>
                                <th>Category</th>
                                <th>Number of Directory</th>
                                <!--<th> Paragraph1 Heading </th>-->
                                <!--<th> Paragraph1 (50-100 words) </th>-->
                                <th>Status</th>
                        ...
                        <tbody>
                            @foreach($col as $key => $blog)
                                   @php
                                      $blogs = \App\Models\CollectionDirectory::where('collection_id',$blog->id)->with('directory')->get();
                                      $item=$blogs->count();
                                    @endphp
                                <tr>
                                    <td>{{ ($col->firstItem()) + $key }}</td>
                                    <td>{{ $blog->meta_key }}</td>
                                    <td>{{ $blog->title }}</td>
                                    <td>{{$blog->suburb  }}</td>
                                    <td>{{ $blog->category }}</td>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="post" action="{{route('admin.collection.data.csv.store')}}" enctype="multipart/form-data" id="fileCsvUploadForm">
                        ...
                        <p><a href="{{URL::to('/')}}/admin/csvexample/collection.csv" target="_blank">
                            <i class="fa fa-download"></i>Download Sample CSV</a></p>
                    </form>
                </div>
            </div>
        </div>
