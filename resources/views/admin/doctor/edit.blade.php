    ...
    <div class="row">
        <div class="col-md-8 mx-auto">
            <div class="tile">
                ...
                <hr>
                <form action="{{ route('admin.doctor.update') }}" method="POST" role="form" enctype="multipart/form-data">
                	<input type="hidden" name="id" value="{{ $targetDoctor->id }}">
                    @csrf
                    <div class="tile-body">
                        <div class="form-group">
                            <label class="control-label" for="name">Title <span class="m-l-5 text-danger"> *</span></label>
                            <input class="form-control @error('title') is-invalid @enderror" type="text" name="title" id="title" value="{{ old('title', $targetDoctor->title) }}"/>
                            @error('title') {{ $message ?? '' }} @enderror
                        </div>
                        <div class="form-group">
                            <label class="control-label" for="name">Type <span class="m-l-5 text-muted">(optional)</span></label>
                            <input class="form-control @error('type') is-invalid @enderror" type="text" name="type" id="type" value="{{ old('type', $targetDoctor->type) }}"/>
                        </div>
                       
                        <div class="form-group">
                            <label class="control-label" for="name">Street Address <span class="m-l-5 text-muted">(optional)</span></label>
                            <input class="form-control @error('street_address') is-invalid @enderror" type="text" name="street_address" id="street_address" value="{{ old('street_address', $targetDoctor->street_address) }}"/>
                        </div>
                        <div class="form-group">
                                        ...
                                        <div class="mb-sm-0 col col-lg fcontrol position-relative filter_selectWrap filter_selectWrap2">
                                            <div class="select-floating-admin">
                                                ...
                                                <select class="filter_select form-control" name="state">
                                                    ...
                                                    <option value="{{$item->name}}" {{ ($item->name == $targetDoctor->state) ? 'selected' : '' }}>{{ $item->name }}</option>
                                                    @endforeach
                                                </select>
                                                @error('state')
                                    ...
                                    <div class="mb-sm-0 col col-lg fcontrol position-relative filter_selectWrap filter_selectWrap2">
                                        <div class="select-floating-admin">
                                            ...
                                            <select class="filter_select form-control" name="postcode">
                                                ...
                                                <option value="{{$item->pin}}" {{ ($item->pin == $targetDoctor->postcode) ? 'selected' : '' }}>{{ $item->pin }}</option>
                                                @endforeach
                                            </select>
                                            @error('postcode')
                                                <p class="small text-danger">{{ $message }}</p>
                        ...
                        <div class="tile-body">
                            <div class="form-group">
                                <div class="select-floating-admin">
                                    ...
                                    <option value="" {{ ($targetDoctor->suburb) ? 'selected' : '' }}>{{$targetDoctor->suburb ?? ''}}</option>
                                    </select>
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control @error('contact') is-invalid @enderror" type="text" name="contact" id="contact" value="{{ old('contact', $targetDoctor->contact) }}"/>
                        </div>
                        <div class="form-group">
                            <label class="control-label" for="email">Email <span class="m-l-5 text-muted">(optional)</span></label>
                            <input class="form-control @error('email') is-invalid @enderror" type="text" name="email" id="email" value="{{ old('email', $targetDoctor->email) }}"/>
                        </div>
                        <div class="form-group">
                            <label class="control-label" for="bathroom">Website <span class="m-l-5 text-muted">(optional)</span></label>
                            <input class="form-control @error('website') is-invalid @enderror" type="text" name="website" id="website" value="{{ old('website', $targetDoctor->website) }}"/>
                        </div>
                        <div class="form-group">
                            <label class="control-label" for="overview">Overview<span class="m-l-5 text-muted">(optional)</span></label>
                            <textarea class="form-control" rows="4" name="description" id="description">{{ old('description', $targetDoctor->description) }}</textarea>
                        </div>
                        {{-- <div class="form-group">
                            <div class="row">
                                <div class="col-md-2">
